...
RSpec.describe CabpoolsHelper, type: :helper do
  ...
  it 'returns the current true if current users cabpool exists' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
  ...
  it 'returns the current false if current users cabpool does not exist' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
  ...
  it 'renders all cabpools except the current users cabpool' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
    ...
    expect(remove_current_users_cabpool(Cabpool.all)).not_to include users_cabpool
  end

  it 'renders all cabpools if current user has no cabpool' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
  ...
  it 'returns requested cabpool of current user' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
  ...
  it 'returns false if current user has not requested for cabpool' do
    user = build(:user)
    names = user.name.split(' ')
    session[:FirstName] = names[0]
    session[:LastName] = names[1]
  ...
  it 'returns false if user has not received response for his/her cabpool request' do
    user = build(:user)
    names = user.name.split(' ')
    user.status = nil
    session[:FirstName] = names[0]
  ...
  it 'returns true if user has received response for his/her cabpool request' do
    user = build(:user)
    names = user.name.split(' ')
  ...
  it 'returns a Requested button which is disabled when the current user has requested for the cabpool' do
  ...
  it 'returns a Leave Ride button which is not disabled when the current user is part of the cabpool' do
    user = build(:user)
    cabpool = build(:cabpool)
    user.cabpool = cabpool
  ...
  it 'returns a Join Ride button which is not disabled the cabpool has seats available' do
    user = build(:user)
    cabpool = build(:cabpool)
  ...
  it 'returns a Ride Full button which is disabled when the cabpool has no seats available' do
    user = build(:user)
    cabpool = build(:cabpool)
  ...
  it 'returns no button when the cabpool does not exists' do
    cabpool_that_does_not_exists = nil
    expect(button_info(cabpool_that_does_not_exists)).to be nil
  end

  it 'returns tw.png when cabpool is of type company provided cab' do
    cabpool = build(:cabpool)
  ...
  it 'returns ola.png when cabpool is of type external cab' do
    cabpool = build(:cabpool)
  ...
  it 'returns carpool.png when cabpool is of type own cab cab' do
    cabpool = build(:cabpool)
  ...
  it 'sorts cabpools based on descending order of available slots' do
    cabpool = build(:cabpool)
  ...
  it 'returns appropriate confirm message when user raises a join request and he is part of a pool' do
    user = build(:user)
    names = user.name.split(' ')
  ...
  it 'returns appropriate confirm message when user raises a join request and he is part of a pool which has no users' do
    user = build(:user)
    names = user.name.split(' ')
    cabpool = build(:cabpool, :without_users)
    cabpool.users = [user]
  ...
  it 'returns appropriate confirm message when user raises a join request for a company provided cab and he is not part of a cabpool' do
    user = build(:user)
    names = user.name.split(' ')
    cabpool = build(:cabpool)
    cabpool.cabpool_type = :company_provided_cab
  ...
  it 'returns the cabpool_type id when the params contains cabpool type information' do
    params[:cabpool_type] = { SomeKey: '2' }
    expect(cabpool_type_that_was_retained).to eq '2'
  end

  it 'returns empty hash when params contains no cabpool type information' do
    params[:cabpool_type] = nil
    expect(cabpool_type_that_was_retained).to be nil
  end

  it 'returns the displayable version of the cabpool ID as the sum of the ID and 100 When id is not nil' do
    cabpool_id = 1

    expect(displayable(cabpool_id)).to eq 101
  end

  it 'returns the displayable version of the cabpool ID as nil id is nil' do
    cabpool_id = nil

    expect(displayable(cabpool_id)).to be nil
  end

  it 'returns cabpool_type symbol of compnay provided cab when id 1 is passed' do
    expect(get_cabpool_type_by_id(1)).to eq :company_provided_cab
  end

  it 'returns cabpool_type symbol of external cab when id 2 is passed as string' do
    expect(get_cabpool_type_by_id('2')).to eq :external_cab
  end

  it 'does not return cabpool_type symbol when id 4 is passed' do
