...
RSpec.describe MailService, type: :service do
  ...
  it 'should call member_addition_to_cabpool once' do
    mail_object = double
    approver = build(:user)
  ...
  it 'should send email to admins requesting creation of cabpool' do
    mail_object = double
    ...
    MailService.send_email_to_admins_to_request_cabpool_creation(requesting_user, timein, timeout, remarks)
    end

  it 'should send an email to the user whose cabpool request has been rejected' do
    mail_object = double
    rejected_user = build(:user)
    expect(mail_object).to receive(:deliver_now)
    expect(CabpoolMailer).to receive(:cabpool_reject_request).and_return(mail_object).once

    MailService.send_email_to_rejected_user(rejected_user)
  end

  it 'should send an email to admin if a user leaves a cabpool and the cabpool is company provided' do
    mail_object = double
    user = build(:user)
  ...
  it 'should not send an email to admin if a user leaves and the cabpool is not company provided' do
    mail_object = double
    user = build(:user)
    another_user = build(:user, :another_user)
    leaving_user = build(:user, :yet_another_user)
  ...
  it 'should send an email to all members of a cabpool on member leaving' do
    mail_object = double
    user = build(:user)
    another_user = build(:user, :another_user)
    leaving_user = build(:user, :yet_another_user)
    ...
    MailService.send_email_to_cabpool_users_on_member_leaving(users, leaving_user)
  end

  it 'should send an email a user who\'s cabpool join request has been approved' do
    mail_object = double
    approved_user = build(:user)
    expect(mail_object).to receive(:deliver_now)
    expect(CabpoolMailer).to receive(:cabpool_approve_request).and_return(mail_object)

    MailService.send_email_to_approved_user(approved_user)
  end

  it 'should send email to all members of a cabpool when it has been edited/updated' do
    mail_object = double
    user = build(:user)
    another_user = build(:user, :another_user)
    ...
    MailService.send_email_to_cabpool_members_about_cabpool_update(updated_cabpool, member_updating_cabpool)
  end

  it 'should send email to all members of a cabpool when a join request has been raised for a non company provided pool' do
    mail_object = double
    digest = double
    user = build(:user)
    another_user = build(:user, :another_user)
    ...
    MailService.send_emails_to_notify_join_request(cabpool, requesting_user, digest)
  end

  it 'should send email to all admins when a join request has been raised for a company provided pool' do
    mail_object = double
    digest = double
    user = build(:user)
    another_user = build(:user, :another_user)
  ...
  it 'should send email to members when admin creates a pool' do
    mail_object = double
    user = build(:user)
    another_user = build(:user, :another_user)
    cabpool = build(:cabpool)
  ...
  it 'should send email to previously existing, current and newly added members of a cabpool when admin updates a cabpool' do
    mail_object = double
